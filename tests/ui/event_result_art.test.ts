...
describe('接線', () => {
  it('單人點選項、連線票結算，兩條路都先等結果圖再跑；進畫面就先抓', () => {
    expect(EVENT).toContain('whenResultArtReady(index, () => take(index));');
    expect(EVENT).toContain('whenResultArtReady(pickIndex, () => take(pickIndex));');
    expect(EVENT).toContain('void preloadEventResults(run, ev.id);');
    expect(EVENT, '單人點選項不能再直接跑結果').not.toMatch(/\n      take\(index\);\n/);
    // 選項的點擊一進來先看是不是還在等
