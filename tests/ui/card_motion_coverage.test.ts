...
class FakeImage {
  ...
  decode(): Promise<void> { decoded.push(this.src); return Promise.resolve(); }
...
describe('新圖還沒下載好：交還靜態立繪，不停在上一個動作的最後一格', () => {
  ...
  it('新圖不進解碼預載（預載完才在背景下載、排進背景解開）', () => {
    ...
    expect(decoded.filter((src) => NEW_TEXTURES.includes(src))).toEqual([]);
  });
});
...
describe('新動作的長度、近戰與命中時間', () => {
  it('照一般速度編寫（跟既有動作同一個尺度），不自己加速', () => {
    expect(qiuqiuMotionDuration('taiji')).toBe(800);
    expect(qiuqiuMotionDuration('qinggong')).toBe(730);
    expect(qiuqiuMotionDuration('focus')).toBe(800);
    expect(qiuqiuMotionDuration('scroll')).toBe(800);
    expect(companionMotionDuration('feifei', 'roar')).toBe(780);
    expect(companionMotionDuration('feifei', 'taiji')).toBe(800);
    // 封封的吼、太極不是劍招，播完不接收劍
    expect(companionMotionDuration('fengfeng', 'roar')).toBe(780);
    expect(companionMotionDuration('fengfeng', 'taiji')).toBe(800);
  });
  ...
  it('都是原地演出，不算近戰；獅吼功命中在吼出來那一拍', () => {
    ...
    for (const kind of ['feifei', 'fengfeng'] as const) {
      ...
      expect(companionImpactTimes(kind, 'roar', 1)).toEqual([380]);
