...
describe('事件畫面', () => {
  ...
  it('抽到之後的那一句接在結果後面（從引擎寫的提示裡找抽到哪一格）', () => {
    expect(src).toContain('const resultText = evText(rawResult) + lotteryAfter(evd.id, notes, me(run, seat).hero);');
  });
...
describe('貓窩、紙箱、獎勵、地圖、狀態列、圖鑑', () => {
  ...
  it('紙箱：單人開完箱才多給、連線兩台在同一拍多給（都接在開箱那一支裡，畫面只拿回報來講）', () => {
    const src = lf(CHEST);
    expect(src).toContain('const id = openChest(run, seat, bonus);');
    expect(src).toContain('const offers: string[] = coop ? openChestCoop(run, bonusAll) : [];');
