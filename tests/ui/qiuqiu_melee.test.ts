...
import { motionMeleePlan, motionMeleeSample, type MeleePoint } from '../../src/ui/qiuqiu-melee';
...
describe('球球近戰瞬移', () => {
  ...
  it('第一格就到目標身旁出招，命中時仍在接觸位置', () => {
    const plan = qiuqiuPlan({ x: 100, y: 500 }, { x: 700, y: 500 }, 200, 'kick');
    const arrived = motionMeleeSample(plan, 0);
    const impact = motionMeleeSample(plan, plan.impactMs);
  ...
  it('收招前留在魔物身旁，結束當下直接回原位', () => {
    const plan = qiuqiuPlan({ x: 100, y: 500 }, { x: 700, y: 440 }, 200, 'attack2');
    const lastStrike = motionMeleeSample(plan, plan.strikeMs - 0.001);
    const done = motionMeleeSample(plan, plan.strikeMs);
  ...
  it('不同敵人位置會產生不同接觸終點', () => {
    const near = qiuqiuPlan({ x: 100, y: 500 }, { x: 600, y: 500 }, 200, 'attack3');
    const far = qiuqiuPlan({ x: 100, y: 500 }, { x: 900, y: 500 }, 200, 'attack3');
    ...
    for (const plan of [near, far]) for (let elapsed = 0; elapsed <= plan.totalMs; elapsed += 10) {
      expect(motionMeleeSample(plan, elapsed).action).not.toBe('run');
    }
  });
  ...
  it('從敵人右側瞬移到左側後，仍面向魔物出招', () => {
    ...
    expect(motionMeleeSample(plan, 0)).toMatchObject({ x: plan.dx, action: 'attack1', facing: 1 });
