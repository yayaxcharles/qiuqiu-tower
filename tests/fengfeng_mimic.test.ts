...
const EXPECT: Record<string, EnemyEffect[] | null> = {
  ...
  fengfeng_hushen: [{ kind: 'block', amount: 7 }],
  fengfeng_tuna: null,
  ...
  fengfeng_tuibu: [{ kind: 'block', amount: 25 }],
  ...
  fengfeng_zhenshou: [{ kind: 'block', amount: 22 }],
  fengfeng_xunxi: null,
...
describe('鏡中影子學封封的牌', () => {
  ...
  it('蓄氣傷害以牌面上限固定，保留多段與穿透；耗氣蜷縮只學自用版', () => {
    expect(learnCard(inst('fengfeng_pingzhan', 1, true))).toEqual([{ kind: 'damage', amount: 20 }]);
    expect(learnCard(inst('fengfeng_chuantang', 1, true))).toEqual([{ kind: 'damage', amount: 29, pierce: true }]);
    expect(learnCard(inst('fengfeng_shuangduan', 1, true))).toEqual([{ kind: 'damage', amount: 12, times: 2 }]);
    expect(learnCard(inst('fengfeng_zhenshou', 1, true))).toEqual([{ kind: 'block', amount: 25 }]);
  ...
  it('消耗全部蓄氣的絕學比照噹噹的卸光不學（使用者 2026-09-21）；有上限的照上限學', () => {
    ...
    expect(learnCard(inst('fengfeng_pozhen', 1, true))).toEqual([{ kind: 'damage', amount: 39 }]);
  });
});
