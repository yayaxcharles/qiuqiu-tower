...
describe('雙人的魔物血量：只放大血量，傷害不動', () => {
  ...
  it('兩個人：一般怪 2.2 倍、菁英 2.4、關主 2.6（殺戮尖塔 2 的真數字）', () => {
    // 2026-09-16 大修：原本填 1.5／1.65／1.75，可是註解寫著「抄的是殺戮尖塔 2」。
    // 查官方 wiki，兩個人的真實倍率是 第一關 ×2.2、第二三關 ×2.4、第三關的王 ×2.6。
    expect(coopHpMul('弱', 2)).toBe(2.2);
    expect(coopHpMul('中', 2)).toBe(2.2);
    expect(coopHpMul('大魔物', 2)).toBe(2.4);
    expect(coopHpMul('塔主', 2)).toBe(2.6);
  });
