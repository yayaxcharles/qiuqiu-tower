...
describe('打一場的獎勵延後', () => {
  it('選項同時有秘寶與打一場：當下不給秘寶，改掛在 fight.afterWin；旗標照常記', () => {
    ...
    const relicsBefore = me(run).relics.length; const notes: string[] = [];
    const out = applyRunEffects(run, choice.outcome, notes);
    expect(me(run).relics.length, '還沒打就不該拿到').toBe(relicsBefore);
    ...
    expect(me(run).relics.length).toBe(relicsBefore + 1);
