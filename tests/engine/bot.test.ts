...
describe('隨機試玩', () => {
  ...
  // 2026-08-30 地圖兩度改版（先改路線式產生、再加不交叉規則），同種子走的路線跟著變，數值重錄兩次。
  it('曾經打超過 60 回合的種子照樣跑得完，而且結果不變', () => {
    expect(playRun('bal-369')).toEqual({ seed: 'bal-369', won: false, floor: 7, turns: 21, kills: 4, deckSize: 12 });
    expect(playRun('bal-453')).toEqual({ seed: 'bal-453', won: false, floor: 12, turns: 50, kills: 9, deckSize: 18 });
  });
});
