...
const setup = (difficulty: number, floor = 44) => {
  ...
  run.act = 3; run.floor = floor; me(run).maxHp = 80; me(run).hp = 30; me(run).fish = 300;
  return run;
};
...
describe('全力準備', () => {
  it('44F、難度 5：升級一張牌、回一成＋魚乾÷10、魚乾歸零', () => {
    const run = setup(5);
    const uid = me(run).deck.find((c) => !c.upgraded)!.uid;
    ...
    expect(me(run).hp).toBe(68);
    expect(me(run).fish).toBe(0);
    expect(me(run).deck.find((c) => c.uid === uid)!.upgraded).toBe(true);
  });
  ...
  it('挑到已升級的牌、不存在的牌：回 false，血與魚乾都不動；舊存檔沒有難度欄＝難度 1 不開放', () => {
    ...
    expect(me(run).hp).toBe(30);
    expect(me(run).fish).toBe(300);
    const old = setup(5);
  ...
  it('難度 3 以下、不是 44F 都不開放；沒挑牌不算，什麼都不動', () => {
    for (const run of [setup(3), setup(5, 43), setup(4, 29)]) {
      ...
      expect(me(run).hp).toBe(30);
      expect(me(run).fish).toBe(300);
      expect(me(run).deck[0]!.upgraded).toBe(false);
    }
    const run = setup(4);
  ...
  it('回血封頂在最大生命，魚乾照樣歸零', () => {
    const run = setup(5);
    me(run).hp = 70;
    rest(run, '全力準備', me(run).deck[0]!.uid);
    expect(me(run).hp).toBe(80);
    expect(me(run).fish).toBe(0);
  });
  ...
  it('44F 打盹照舊回滿、不動魚乾；磨爪照舊只回一成', () => {
    const a = setup(5);
    rest(a, '打盹');
    expect(me(a).hp).toBe(80);
    expect(me(a).fish).toBe(300);
    const b = setup(5);
    rest(b, '磨爪', me(b).deck[0]!.uid);
    expect(me(b).hp).toBe(38);
    expect(me(b).fish).toBe(300);
  });
});
