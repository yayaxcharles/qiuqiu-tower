...
describe('狀態效果', () => {
  ...
  it('中毒發作：回傳該掉的血、層數減一（扣血由呼叫端走 damageEnemy／damagePlayer，審查 #10）', () => {
    const u = blankUnit(20); u.block = 5; addStatus(u, '中毒', 3);
    expect(tickPoison(u)).toBe(3);
    expect(u.hp).toBe(20); expect(u.block).toBe(5); expect(getStatus(u, '中毒')).toBe(2);
