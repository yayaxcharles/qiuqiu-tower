...
describe('忍具格', () => {
  ...
  it('換掉第 index 支', () => {
    ...
    expect(me(r).potions).toEqual(['whetstone', 'claw_oil', 'whetstone']);
    expect(replacePotion(r, 5, 'claw_oil')).toBe(false);
    expect(replacePotion(r, 0, 'no_such')).toBe(false);
  });
  it('罐頭鋪帶滿：沒指定要換哪支就不賣也不扣錢；指定了才換', () => {
    const r = newRun('shop-pot'); me(r).fish = 500;
    addPotion(r, 'whetstone'); addPotion(r, 'whetstone'); addPotion(r, 'whetstone');
    const shop = makeShop(r);
    const fish = me(r).fish;
    expect(buyPotion(r, shop, 0)).toBe(false);
    expect(me(r).fish).toBe(fish);
    expect(buyPotion(r, shop, 0, 2)).toBe(true);
    expect(me(r).fish).toBe(fish - shop.potions[0]!.price);
    expect(me(r).potions[2]).toBe(shop.potions[0]!.id);
  ...
  it('戰鬥獎勵帶滿：收不下的那支記在 potionMissed', () => {
    ...
    const cs = startCombat({ hp: 70, maxHp: 70, deck: STARTER_DECK.map((id, i) => inst(id, i + 1)), relics: [], potions: [...me(r).potions], encounterId: 'cucumber', rng: new Rng(seedFromString('x')) });
