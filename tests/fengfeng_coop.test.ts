...
describe('封封 FG-T13～FG-T17、FG-T21', () => {
  it('FG-T13：兩種支援取大；不能提高時出牌前拒絕且不支付', () => {
    ...
    expect([p.nextAttackBonus, q.nextAttackBonus]).toEqual([6, 6]);
    p.qi = 3; play(cs, p, 'fengfeng_youbian');
    expect(q.nextAttackBonus).toBe(9);
  ...
  it('FG-T14：雙封封各付各的蓄氣，加成分席保存', () => {
    ...
    expect([p.nextAttackBonus, q.nextAttackBonus]).toEqual([7, 9]);
  });

  it('FG-T15：單人取得四張連線牌時都依自用規則，雙人加成不會複製', () => {
    const { cs, p } = setup(false);
    p.qi = 3; play(cs, p, 'fengfeng_youbian'); expect(p.nextAttackBonus).toBe(9);
    p.nextAttackBonus = undefined; p.block = 8; p.qi = 0; play(cs, p, 'fengfeng_jiewo'); expect(p.qi).toBe(5);
    p.block = 0; p.qi = 3; play(cs, p, 'fengfeng_husong'); expect(p.block).toBe(13);
    p.qi = 4; play(cs, p, 'fengfeng_yiqichushou'); expect(p.nextAttackBonus).toBe(6);
  });
  ...
  it('FG-T16：已結束拒收、倒下改自用、存活席位仍是隊友', () => {
    ...
    play(down.cs, down.p, 'fengfeng_youbian'); expect(down.p.nextAttackBonus).toBe(9);

    const alive = setup(); alive.p.qi = 3;
    play(alive.cs, alive.p, 'fengfeng_youbian'); expect(alive.q.nextAttackBonus).toBe(9);
  ...
  it.each([0, 2])('原樣奉還有 %i 點反彈時，支援只加入原牌已有的傷害', (thorns) => {
    ...
    expect(hp - enemy.hp).toBe(thorns > 0 ? thorns * 2 + 9 : 0);
