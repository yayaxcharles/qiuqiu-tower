...
const MUST: string[] = [
  ...
  '中毒', '反彈', '定身', '壞毛病', '小魚乾', '秘寶', '忍具', '罐頭鋪', '貓窩', '紙箱',
...
describe('名詞表', () => {
  ...
  it('蓄氣說明包含上限、保留、消耗與歸零時機', () => {
    expect(glossary['蓄氣']).toContain('最多 12 點');
    expect(glossary['蓄氣']).toContain('跨回合保留');
    expect(glossary['蓄氣']).toContain('才會消耗');
    expect(glossary['蓄氣']).toContain('倒下或戰鬥結束時歸零');
  });
});
