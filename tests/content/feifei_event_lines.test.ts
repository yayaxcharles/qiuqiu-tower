...
describe('共用事件台詞的她版', () => {
  ...
  /*
   * **刻意不要求「一句都不能漏」**：88 句裡使用者看過之後留了 6 句維持原樣
   *（像「慢慢走，牠們就在那邊喵。」——本來就沒有球球的嘴硬，拿掉喵她講也通）。
   * 硬性全覆蓋會逼人為了讓測試變綠去改那 6 句，那是本末倒置。
   * 這裡守的是「絕大多數有改」，新加一整批事件卻整批沒寫她的版本會被抓到。
   */
  it('至少九成有她自己的版本', () => {
    const missing = theirs.filter((t) => FEIFEI_EVENT_LINES[t] === undefined);
    expect(missing.length / theirs.length,
      `沒寫她的版本的有 ${missing.length}／${theirs.length} 句：\n  ${missing.join('\n  ')}`).toBeLessThan(0.1);
  });
