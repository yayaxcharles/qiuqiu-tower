...
describe('秘寶', () => {
  it('66 件、池數正確、id 不重複', () => {
    expect(relics.length).toBe(66);   // 60＝2026-09-02 擴充（36 → 60）；65＝2026-09-04 五件代價秘寶；66＝2026-09-12 菲菲的起始秘寶（2026-09-13 改名毒針袋）
    const n = (p: string) => relics.filter((r) => r.pool === p).length;
    expect(n('起始')).toBe(2);   // 藍頭巾（球球）＋毒針袋（菲菲）
    expect(n('常見')).toBe(30); expect(n('大魔物')).toBe(24); expect(n('塔主')).toBe(10);   // 2026-09-04 代價秘寶：常見 +2、大魔物 +3
