...
import { combatFingerprint } from './hash';
...
export interface SequencedAction { seq: number; a: CoopAction }
...
export class Sequencer {
  ...
  assign(a: CoopAction): SequencedAction {
    this.n += 1;
    return { seq: this.n, a };
  }
...
export interface ApplyResult {
  /** 這一次真正套進去的（照編號順序） */
  applied: SequencedAction[];
  /** 引擎拒絕的那一個。出現這個就代表兩邊的狀態已經對不上了 */
  failed?: SequencedAction;
...
export class ActionQueue {
  /** 下一個要套用的號碼 */
  private next = 1;
  private buffered = new Map<number, CoopAction>();
   ...
   */
  receive(cs: CombatState, sa: SequencedAction): ApplyResult {
    ...
    const applied: SequencedAction[] = [];
    while (this.buffered.has(this.next)) {
      const a = this.buffered.get(this.next) as CoopAction;
      this.buffered.delete(this.next);
      const one: SequencedAction = { seq: this.next, a };
      if (!applyAction(cs, a)) {
...
/** 每回合結束互相對一次的訊息 */
export interface SyncCheck { turn: number; fp: string }
