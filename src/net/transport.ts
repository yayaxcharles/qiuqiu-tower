...
export type NetMessage =
   ...
   */
  /*
   * `f`＝**這個動作是第幾場戰鬥的**（2026-09-14 夜間稽核 高-6）。
   *
   * 兩台進場的時間本來就不一樣（對白、開門、事件的「開打」各點各的，差好幾秒），
   * 沒有這個欄位的話，先進場的那台出的牌會被還停在上一場的那台套進**上一場**、當場斷線；
   * 上一場最後一刻送出、到得太晚的牌，也可能被套進下一場（牌號每場都從牌組複製，會撞號）。
   ...
   */
  | { m: 'drop'; n: number }
  | { m: 'act'; seq: number; a: CoopAction; f?: number }
  /**
