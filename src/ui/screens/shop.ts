...
import { RESHUFFLE_COST, buyCard, buyPotion, buyRelic, buyRemove, buySwap, canSwap, keeperMulFor, makeShops, notMyCard, potionCapacity, priceFor, removePrice, reshuffleShop, shopMulFor, shopService, type ShopStock } from '../../engine/run';
...
registerScreen('shop', (app, root, props) => {
  ...
  function keeperArt(): string | undefined {
    // 新表情沒生好就退回招呼那張，退不了才整個不放（跟原本一樣）。鍵照這位店主（`shop/keeper_tortoise_happy` → `shop/keeper_tortoise`）
    for (const key of mood === 'idle' ? [K.art] : [`${K.art}_${mood}`, K.art]) {
  ...
  function priceNode(price: number, sold: boolean, base?: number, sale?: number, soldText = '賣掉了', item?: object): HTMLElement {
    ...
    const orig = base === undefined ? price : Math.round(base * shopMulFor(run, seat) * Math.max(1, item ? keeperMulFor(shop, item) : 1));
  ...
  function leaveBtn(): HTMLElement {
    if (!coop) return el('button', { class: 'btn primary', onclick: () => { app.backToMap(); sayLeave(); } }, '離開');
  ...
  function render(): void {
    ...
    shop.cards.forEach((it, i) => {
      ...
      const slot = el('div', { class: `shop-item card-item${it.sold ? ' sold' : buyable ? '' : ' poor'}${it.sale && !it.sold ? ' on-sale' : ''}` },
        ...
        theirs && !it.sold ? el('div', { class: 'price' }, '同伴的牌') : priceNode(price, it.sold, it.base, it.sale, undefined, it));
    ...
    shop.relics.forEach((it, i) => {
      ...
      relics.append(stall(d.art, d.name, relicLongText(d, me(run, seat).relics), priceFor(run, it, seat, shop), it.sold || owned, false,
        ...
        !it.sold && owned ? '你已經有了' : undefined, undefined, !!it.limited, it));
    });
    ...
    shop.potions.forEach((it, i) => {
      ...
      potions.append(stall(d.art, d.name, full ? `${d.text}（帶滿了，買了要換掉一支）` : d.text, price, it.sold, poor,
        ...
        }, it.base, it.sale, undefined, d.rarity, undefined, it));
    });
    ...
    root.append(sceneView({
      ...
      actions: [reshuffle, remove, serviceBtn(), leaveBtn()],
    }));
  }
  ...
  if (coop) {
    coop.onRunApplied((applied) => {
      ...
      if (allDone()) { app.backToMap(); sayLeave(); return; }
      render();
    });
  }
