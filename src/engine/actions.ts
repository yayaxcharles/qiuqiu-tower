 ...
 *
 * 注意有的效果自己也會印一行（走 `selfDamage` 的代價型秘寶，`effects.ts` 對秘寶來源的自傷
 * 會印「秘寶的代價：失去 N 點生命」），那種就會是兩行——那是對的，代價本來就該講清楚。
 * （2026-09-11 起沒有秘寶在用那條路：鐵砂衣的扣血被拿掉了。護欄與這段說明都留著備用。）
 *
 ...
 */
/**
 * 兩個人時擋下的紀錄要寫是誰的（使用者 2026-09-15：「蜷縮明明不夠，打下去卻擋住了」——擋的是同伴那一下，
 * 魔物每一招隨機打一位，紀錄沒寫名字就像自己擋的）。單機照舊不加。
 */
...
function killEnemy(cs: CombatState, e: EnemyCombat, by?: PlayerCombat): void {
  ...
  // 擊倒者已經倒下就不發（跟下面的秘寶同一條，推前審查 2026-09-16 低-2：回復卷軸讓躺著的人血從 0 變 3）
  if (!reviving && !killer.down) for (const pw of killer.powers) if (pw.trigger === 'onKill') applyEffects(cs, pw.effects, { self: killer, source: 'power' });
  /*
