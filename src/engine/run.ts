...
function enterEvent(run: RunState, n: MapNode): void {
  ...
  if (pending && pending.id !== cur && !freshSequel && !eventById[cur]?.rare) {
...
export function openChest(run: RunState, seat = 0, bonus?: { seat: number; id: string }[]): string | null {
  const rng = runRng(run);
 ...
 * 這樣「一定有東西」的保證對兩個人都成立。
 */
export function openChestCoop(run: RunState, bonus?: { seat: number; id: string }[]): string[] {
  const rng = runRng(run);
...
/** 這一格罐頭鋪是誰顧的（店主輪替那條線寫在 `MapNode.keeper`；還沒有那一欄的地圖＝橘貓老闆） */
function keeperOf(n: MapNode): string {
  return (n as MapNode & { keeper?: string }).keeper ?? 'orange';
}
/** 這一位集章卡上蓋了幾個章（沒帶就是 0） */
