...
export const events: EventDef[] = [
  ...
  { id: 'rescue', title: '江湖救急',
    text: '球球把受傷的村貓救了下來。牠趴在地上喘著氣，把身邊的一包小魚乾和一罐貓草藥推了過來：「謝謝你救了我的命，請讓我給你謝禮。」',
    choices: [
      { label: '拿貓草藥（回復 20 點生命）', outcome: [{ kind: 'flag', name: 'rescue_took_herb' }, { kind: 'heal', n: 20 }], result: '藥很苦。球球：「苦的才有效喵。」' },
      { label: '拿小魚乾（拿 40 條小魚乾）', outcome: [{ kind: 'flag', name: 'rescue_took_fish' }, { kind: 'fish', n: 40 }], result: '村貓揮手要球球快走。球球：「我一定會回來救你的喵。」' },
    ] },
  ...
  { id: 'moving_rat', title: '換家的老鼠', acts: [2, 3],
    text: '一隻背著大包袱的老鼠蹲在階梯上清點家當。「搬家嘛，什麼都嫌重。」牠瞄了球球身上一眼，「你那件……跟我換兩件？我隨手抓，你也隨手拿，公平。」',
    choices: [
      // 交一件換兩件：期望值是賺的，但**交出去的那件是隨機的**——你可能失去這一局的核心。
      // 那個不確定性才是這個事件的重點，所以不讓玩家挑要交哪一件
      { label: '跟牠換（隨機交出一件秘寶，換兩件常見秘寶）',
        outcome: [{ kind: 'loseRelic' }, { kind: 'relic', pool: '常見' }, { kind: 'relic', pool: '常見' }],
        result: '老鼠伸手在球球身上摸了一把，又從包袱裡掏了兩樣塞過來。球球：「等等，我那個——算了喵。」' },
      { label: '不換', outcome: [], result: '老鼠聳聳肩，把包袱又背回去。球球：「我的東西我自己拿著喵。」' },
    ] },

  { id: 'grindstone', title: '磨到只剩一把刀', acts: [2, 3],
    text: '一塊被磨得凹下去的老磨刀石靠在牆邊，旁邊刻著一行字：「刀多不如刀利。」',
    choices: [
      // 一次砍三張是這個遊戲裡最大的一刀「瘦牌組」。代價放在最大生命而不是當下的血：
      // 扣血可以睡一覺補回來，扣上限才是真的付出去了
      { label: '磨（放生三張牌，最大生命 −8）',
        outcome: [{ kind: 'removeCard' }, { kind: 'removeCard' }, { kind: 'removeCard' }, { kind: 'maxHp', n: -8 }],
        result: '球球磨到天亮，手上只剩下最順的那幾張。球球：「留著的都是會用的喵。」' },
      { label: '不磨', outcome: [], result: '球球摸了摸石頭就走了。球球：「我的牌每一張都有用喵。」' },
    ] },

  { id: 'shortcut_scroll', title: '速成的卷軸', acts: [1, 2, 3],
    text: '牆上釘著一卷攤開的卷軸，字寫得又急又亂：「三日速成，勿問後果。」墨還沒乾。',
    choices: [
      // 升級兩張是很大的加強，代價是一張壞毛病——「速成」的意思就是欠著，之後每一場都要還
      { label: '照著練（升級兩張牌，但學到一個壞毛病）',
        outcome: [{ kind: 'upgradeCard' }, { kind: 'upgradeCard' }, { kind: 'addRandomCard', pool: '壞毛病' }],
        result: '球球照著練了一遍，招式利落了不少，但總覺得哪裡怪怪的。球球：「後果……應該還好吧喵？」' },
      { label: '不練', outcome: [], result: '球球把卷軸捲回去。球球：「墨都還沒乾，誰知道是誰寫的喵。」' },
    ] },
];
