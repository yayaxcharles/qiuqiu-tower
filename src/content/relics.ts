...
export const relics: RelicDef[] = [
   ...
   * 另外三隻自己的起始秘寶值 +3.4～4.8 層。加開場 1 點爪力（事件「欄杆上的藍頭巾」那句「綁緊一點，出爪就更有力了」），
   * 量到 +3.2 層：比另外三隻略低一點，因為球球的牌本來就撐得住，補滿的話他會跟菲菲並列最高（量尺見 `docs/秘寶量尺.md`）。
   */
  { id: 'blue_headband', name: '藍頭巾', pool: '起始', text: '每場戰鬥開始時獲得 1 點爪力，第一回合多抽 1 張牌。', art: 'codex/relic_headband', price: 130,
    hooks: { firstTurnDraw: 1, combatStart: [{ kind: 'status', name: '爪力', amount: 1, target: 'self' }] } },
  /*
  ...
  /*
   * 噹噹的起始秘寶（2026-09-17）。對照：球球＝藍頭巾（第一回合多抽一張；09-23 起另給 1 點爪力）、菲菲＝毒針袋（開場全體 3 層毒）。
   *
  ...
  /*
   * 2026-09-23 平衡（bal）：10 → 15 條。量尺 +0.2～0.9 層，但小魚乾量尺量不準（機器人逛店規則簡單，錢花不滿）；
  ...
  /*
   * 2026-09-23 平衡（bal）：毛線球、九尾墜等於「每回合多 1 顆飯糰」、量到多爬 9 層，比塔主池一半以上還強，
   * 大魔物獎勵抽到它跟抽到別件差太多。只收一點：第一回合少 1 顆（等於第二回合起才賺），量到 +7.3 層，仍是大魔物池前段。
   */
  ...
  // 2026-09-23 平衡（bal）：原本兩成機會抽 1 張，量尺 +0.1～0.7 層（多抽的牌多半打不出去，飯糰才是瓶頸）；改成一成半機會抽牌＋飯糰，+2.6 層＝大魔物池中下
   ...
   * 加「用完忍具得 6 點蜷縮」：量到 +1.2 層＝常見池下四分位；忍具類量尺偏低（真人會留、會挑時機喝），心算落在中下。
   */
  ...
  /*
   * 2026-09-23 平衡（bal）：量尺 +0.3～0.9 層（一局只打盹兩三次，每場約 0.1 次發動）；蜷縮加到 20、25 也只 +1.1～1.2。
   * 改成打盹時多回 4 點＋下一場 20 點蜷縮：+1.9 層＝常見池中位附近，還是「打盹的秘寶」
   */
  ...
  // 2026-09-23 平衡（bal）：2 層隱身量到 +8.9 層（大魔物池上四分位 4.5）；第二層換成 1 點貓步，+7.4 層，仍是池子前段
  ...
  // 2026-09-23 平衡（bal）：第 3 張抽 1 張量尺 ≈0（打到第三張時飯糰多半用完，抽到的打不出去）；改成第 2 張就抽 3 張，+2.5 層＝大魔物池中下
  ...
  // 2026-09-23 平衡（bal）：跟塔主令牌一樣「每回合多 1 顆」、量到 +9.2 層；只收一點＝第一回合不加（上限 +1、第一回合 −1），+7.3 層
  ...
  // --- 常見（+10）---
  // 2026-09-23 平衡（bal）：只多抽 1 張量尺 ≈0（抽 2 張也只 +0.4）；加下回合多 1 顆飯糰（忍一回合、下回合爆發），+1.9 層＝常見池中位附近。
  ...
  // 2026-09-23 平衡（bal）：3 → 2 點，+5.4 → +4.3 層（常見池上四分位 2.9，仍是前段）
   ...
   * 改成第 4 張時抽 1 張、多 2 顆：+1.4 層＝常見池下四分位之上。改第 3 張會跳到 +3.9 層（金爪套的一半），常見池太強
   */
  ...
  // 2026-09-23 平衡（bal）：3 → 2 點，+7.2 → +5.3 層（大魔物池上四分位 4.5），仍是前段
  ...
  // 2026-09-23 平衡（bal）：只靠打倒魔物長爪力量尺 +0.9 層（多半是最後一隻倒下才長，這場用不到）；打倒 2、3 點也只 +1.3～1.7。
  // 加開場 1 點爪力：+3.1 層＝大魔物池中位附近
  ...
  // 2026-09-23 平衡（bal）：只抽 2 張量尺 +0.6 層；加 2 顆飯糰，+2.6 層＝大魔物池中位（忍具類量尺偏低，真人喝得比機器人有計畫）
  ...
  /*
   * 2026-09-23 平衡（bal）：8 → 15 條。量尺 +0.5 層（12 條 +0.5～1.0、15 條 +1.3；小魚乾量尺量不準，照心算定）。
  ...
  // --- 塔主（+5）---
  // 2026-09-23 平衡（bal）：量到 +11.9 層、塔主池第二強；只收一點：貓步 3 → 2，+11.0 層
  ...
  /*
   * 2026-09-23 平衡（bal）：只有「牌多一張可選」量尺 ≈0（−0.3～+0.3 層；挑牌類量不準，但塔主三選一裡它等於廢選項）。
   * 加開場「全體 1 層懶洋洋＋1 層翻肚」（掌門印一亮，魔物先軟了腳）：+3.6 層＝塔主池下四分位到中位之間
   */
  ...
  // 2026-09-23 平衡（bal）：每回合 8 量到 +13.6 層、全遊戲最強（過第二關多 48 個百分點）；只收一點 8 → 6，+12.1 層，仍是塔主池第一
  ...
  // 2026-09-23 平衡（bal）：代價 2 → 3 層炸毛，+6.7 → +6.2 層（跟九尾墜、毛線球同一群，仍是前段）
  ...
  // 改成給潛水（下回合開始才變隱身，竹筒、影忍頭帶同一套），+7.8 層，只收一點
  ...
  // 2026-09-23 平衡（bal）：一個人時每回合 2 點給自己＝鐵砂袋（大魔物）打七折，量到 +5.3 層、常見池第二強；2 → 1 點，+3.2 層（仍在上四分位之上）
  ...
  // 2026-09-23 平衡（bal）：門檻 10 量尺還是 +0.4 層（降到 8、6 也只 +0.6～0.7），缺的是氣；加每回合 1 點蓄氣，門檻照 10，+3.7 層＝塔主池中位附近
  ...
  // 2026-09-23 平衡（bal）：另外三隻自己不太下毒，量到 +0.6～0.8 層（塔主三選一裡等於少一個選項）；
  // 開場給全體 1 層毒當引子（這一層也吃「多扣 2 點」），四隻平均 +3.0 層、菲菲 +5.2
  ...
  // 2026-09-23 平衡（bal）：只有球球常閃，另外三隻量到 +0.6～0.8 層；加開場 1 點貓步（誰都用得到的身法），四隻平均 +4.0 層、球球 +5.9
