...
標題畫面按「🤝 兩個人一起玩」。**兩台機器直接連線，中間不經過任何伺服器**，所以要互相貼一次代碼：
開房的人先產生一串「邀請碼」傳給對方（用 LINE 傳就好），對方貼進去產生「回應碼」傳回來，貼上按「連上」就通了。

規則（跟單機不一樣的地方）：

- **各帶各的**：血量、牌組、秘寶、忍具、小魚乾都是一人一份，互不相通。
- **魔物一招隨機打一個人**。有人幫忙擋（連線牌「我來擋」那類）時會先打擋的那位。
- **魔物的血量 ×1.5**（大魔物 ×1.65、塔主 ×1.75），不是 ×2——兩個人的工作量沒有變兩倍。
- **回合要兩個人都按「結束回合」才收**。舉了手還能按「再想想」收回來；對方超過一分鐘沒動作，
  你這邊會多一顆「替他收回合」。
- **選路線兩個人各選一格**：選一樣就走那一格，選不一樣就擲一次骰。事件的選項也是這樣。
- **獎勵分開給**：小魚乾與忍具兩個人各拿一份全額，牌是同一份戰利品各挑一張，
  秘寶則是開兩件各挑一件（挑到同一件就擲骰決定誰拿到，另一個人拿剩下那件）。
- **罐頭鋪的貨架共用、錢包各自**：同一格只賣得掉一次，誰先買到算誰的；折扣秘寶只影響自己的售價。
- **一人倒下，另一個人繼續玩到輸為止**；倒下的人看得到對方在做什麼，要在貓窩讓隊友「扶起同伴」
  才會回三成血站起來（扶人的那位就用掉自己這一格的打盹機會）。
- **只有連線時才會出現的牌**（白色牌底）：幫對方擋、幫對方隱身、分飯糰給對方那一類，
  商店與戰鬥獎勵都抽得到。

做法是**鎖步連線**：兩邊各跑一份一模一樣的引擎，線上只傳「誰做了什麼」，不傳遊戲狀態。
所以引擎必須完全決定性（`src/engine/` 底下沒有 `Math.random`、不讀時間），
每回合與每走一格都互相對一次指紋，對不上就當場停下來、在畫面上方壓一條紅色橫幅說明，
不會讓兩個人默默玩兩份不一樣的遊戲。

網址後面加 `?debug` 會把 `app` 掛到瀏覽器主控台的 `__app`，方便查連線的狀況（一般玩家碰不到）。
