...
# 這幾張是球球的舊圖，原稿（codex_raw／art_inbox）都找不到了，重做要連提示詞一起重建。
# 列在這裡是**已知未修**，不是放過——要修的話得先重生原稿。
KNOWN = {
    "event_broken_shrine_r1.webp", "event_rescue_return_fish_r1.webp",
    "event_sparring_cat_r1.webp", "event_stuck_kitten_r0.webp",
    "event_toll_again_paid_r0.webp", "event_training_hall_r0.webp",
    "event_weapon_rack_r1.webp",
}
...
def main() -> None:
    ...
    for p in sorted((ROOT / "public" / "assets" / "bg").glob("*.webp")):
        ...
        if corner_haze(Image.open(p)):
            (known if p.name in KNOWN else bad).append(p.name)

    if known:
        print(f"已知未修（球球的舊圖，原稿不見了）{len(known)} 張：{'、'.join(known)}")
    if bad:
        print(f"!! 新的灰膜 {len(bad)} 張，進倉時重做一次：")
        for b in bad:
            print(f"   python tools/add_event_art.py --soft 190 --hard 230 {b[:-5]}.png")
            print(f"   （還是不行就改用 --strict，那是給「主體本身有大片亮色光」的圖用的）")
        raise SystemExit(1)
    print(f"沒有新的灰膜{'（只掃了含 ' + needle + ' 的）' if needle else ''}")
