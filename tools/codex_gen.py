...
1. **一次只跑一批。** 單張約 3～7 分鐘；同時開兩三批會互相搶，實測單張跑滿 20 分鐘還沒好。
   ...
   **2026-09-12 換模型之後又量了一次，結論一樣。** 那天 fast 模式把單張從 400 秒壓到 90 秒，
   看起來像是「機器變快了，應該開得起平行」，所以開了四條線同時跑（主線＋三份共用牌＋一批事件圖）：

   | 線 | 十分鐘內產出 |
   |---|---|
   | 主線（本來就在跑的） | 4 張，84～118 秒／張，完全沒受影響 |
   | 另外四條 | **0 張**，全部卡在第一張 |

   **瓶頸在 Codex 那端排隊，不在本機。** 換模型改善的是「單張多快」，
   不是「能同時跑幾張」——這兩件事不要混在一起想。

   同一天再量了一次，比較精確的結論是：**兩條可以，四條會餓死。**
   收掉多餘的之後留兩條（主線＋一批小工單），兩條都在產，只是第二條慢一截
   （主線 93 秒／張，第二條 192 秒）。所以兩條的總產出大約是一條的 1.3 倍，不是 2 倍。
   **要插隊做一小批急件時開第二條是划算的；想靠開四條把時間砍成四分之一是不可能的。**

   **更正（同一天稍晚，把整份紀錄排成時間線之後）**：上面那段的歸因是錯的。
   最快的一段（76～108 秒）正好是開了四條平行線的時候，而收到只剩一條之後照樣有 270 秒。
   **速度跟開幾條線對不上，是服務端自己在波動。** 留著「一次只跑一批」這條規矩仍然合理
   （多開沒有好處、還會讓每條都看不出在跑還是卡住），但**不要再拿它解釋忽快忽慢**。

   還有一個真的會害人的：**停掉一條線要砍到 Python 行程，不是砍包它的那層腳本。**
   用 bash 包一層再背景執行時，停掉背景工作只停到 bash，裡面的 `python codex_gen.py`
   還活著。要確認真的只剩一條：
       Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
         Where-Object { $_.CommandLine -match 'codex_gen' }
   看到幾條就是幾條，不要相信「我應該停掉了」。

   所以要更快只有兩條路：**把圖變少**（重用提示詞、只生真的會用到的），
   或**接受它就是要跑這麼久**，排成一條線去做別的事。

   **停掉一條線要砍到 Python 行程，不是砍包它的那層腳本。** 2026-09-12 同一天踩到：
   用 bash 包一層再背景執行，停掉背景工作時只停到 bash，裡面的 `python codex_gen.py`
   還活著繼續搶——量到的「一條線也很慢」其實是「以為收掉了但沒收掉」。
   要確認真的只剩一條，用行程清單看命令列：
       Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
         Where-Object { $_.CommandLine -match 'codex_gen' }
   看到幾條就是幾條，不要相信「我應該停掉了」。
