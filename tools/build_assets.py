...
build_assets.py — 從 Dropbox 貼圖原圖產出遊戲素材（只產遊戲真的會用到的鍵）。
  ...
  立繪：裁掉頂端標題帶 → 高 512 WebP → public/assets/sprites/<set>/<NN>.webp
        ⚠ **2026-08-30 起整支腳本的產出都沒人用了。**
          主角與塔主換成專畫的 hero/*、boss/*；58 張牌面換成專畫的 card/*，
          都改由 tools/build_art_inbox.py 產出。這裡的 cards/ninja、cards/daxia、
          sprites/ninja、sprites/daxia 共約 3.3 MB 已全數下架。
          腳本留著是因為貼圖來源與裁切邏輯（標題帶、連通元件）還有參考價值，
          但**重跑會把那 3.3 MB 寫回去、也會把 manifest 補回那些鍵**，跑完記得再刪一次。
