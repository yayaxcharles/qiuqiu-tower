...
describe('素材清單的衛生', () => {
  ...
  it('事件插圖沒有沒人用的孤兒', async () => {
    ...
    // 問號格變化的三張揭曉圖（2026-09-23 第三批）：不是 `EventDef`，編號照引擎那一份（`QMARK_ART`），畫面照它叫 `eventArtKey`
    const { QMARK_ART } = await import('../src/engine/qmark');
    for (const k of Object.values(QMARK_ART)) ids.add(k);
